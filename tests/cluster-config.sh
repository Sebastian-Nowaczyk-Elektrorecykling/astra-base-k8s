...
trap 'rm -rf -- "$tmp"' EXIT
mkdir -p "$tmp/scripts" "$tmp/clusters/laptops"
...
before=$(kubectl patch --local --type=merge --patch '{}' -f "$settings" -o json | jq -S 'del(.data.API_HOST, .data.POD_CIDR)')
...
EOF
  ...
  jq -e --arg host "$host" '.data.API_HOST == $host and .data.POD_CIDR == "10.88.0.0/16"' <<<"$updated" >/dev/null
  [[ $(jq -S 'del(.data.API_HOST, .data.POD_CIDR)' <<<"$updated") == "$before" ]]
