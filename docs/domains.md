...
Configure records on your existing router or authoritative LAN resolver. Publish exact machine records for `k8s1`, `k8s2` and `k8s3`, using their actual addresses. Publish the four application wildcards `*.internal`, `*.admin.internal`, `*.test.internal`, `*.staging.internal` to the configured private gateway address. The repository does not install a DNS server or publish DNS records.

Reserve `hosts.internal` as a separate authoritative zone/subtree containing only machine records. Unknown machine names must return NXDOMAIN, not fall through a resolver's broad wildcard forwarding rule to `EDGE_IP`. How to configure this depends on your existing resolver; test it explicitly. Never point `*.hosts.internal` at the gateway. Configure laptops, workstations and the nodes' upstream DNS to use the LAN resolver, including over VPN. Applications using encrypted/public DNS may need an internal-zone exception. `/etc/hosts` can bootstrap a few exact names but cannot implement wildcard DNS.
