...
4. A client-valid TLS certificate: grouped test/staging/admin names already have wildcards, but direct `foo.internal` names require [an exact SAN in the base profile](tls.md#exact-names-for-applications-directly-under-internal). Add that reviewed base change alongside the downstream route; an accepted route does not prove the certificate matches.
...
The included policy implements the autonomous-agent/service-entry case. Fine-grained delegation and gateway business operations require the future **application's existing supported integration** or application logic. They are not represented as solved by a generic ingress rule, and no custom foundation service is created here. Authenticated headers alone are not a permission system. Keep native integrations on distinct clients/audiences when designing their access. OAuth cookies remain host-only; treat all request cookies as sensitive and avoid logging them. Verify the header replacement against the upstream before onboarding an untrusted application.
