...
- [Cilium BGP](https://docs.cilium.io/en/stable/network/bgp-control-plane/bgp-control-plane-configuration/) and [LB IPAM](https://docs.cilium.io/en/stable/network/lb-ipam/) allocate off-link service VIPs and advertise only private DNS/gateway host routes to the LAN router. L2 is disabled; `Cluster` traffic policy and SNAT let controller peers forward to worker backends.
...
- [Kubernetes validating admission policies](https://kubernetes.io/docs/reference/access-authn-authz/validating-admission-policy/) provide in-process security guards; [Kyverno MutatingPolicy](https://kyverno.io/docs/policy-types/mutating-policy/) supplies storage defaults using the stable `policies.kyverno.io/v1` API. The pinned Kyverno 1.19 deprecates ClusterPolicy; the base uses its supported CEL replacement. See [upstream policy guidance](https://kyverno.io/docs/guides/migration-to-cel/). The baseline contains no OPA server, handwritten authorization microservice, Crossplane composition, or Kratix controller.
