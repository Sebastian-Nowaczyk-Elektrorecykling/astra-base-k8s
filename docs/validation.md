...
The secret-generation regression injects a failed SOPS encryption, verifies no target/plaintext remains, retries using real SOPS/age/kubectl, checks shared OIDC credentials and the Authorino Secret label, and verifies existing credentials and unrelated Kustomize resources are preserved. These are temporary test credentials only. Installer checks reject invalid explicit IPv4 addresses and oversized node names before any host installation.
