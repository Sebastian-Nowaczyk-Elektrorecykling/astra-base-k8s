...
CNPG's default is applied to the Cluster CR by Kyverno's `policies.kyverno.io/v1` MutatingPolicy, not to every PVC carrying a guessed label. Explicit nonempty data/WAL classes are respected. Adding `walStorage` without a class also gets `longhorn-cnpg`. This does not move existing PVCs or mutate an immutable PVC class; for existing databases use a supported CNPG migration/recovery operation. An independent native Kubernetes validation rule requires nonempty data/WAL classes after mutation. Missing defaults are rejected even if Kyverno's webhook is unavailable or removed; a CNPG request cannot silently fall through to the ordinary PVC default. Explicitly chosen classes remain valid choices when the mutator is disabled.
