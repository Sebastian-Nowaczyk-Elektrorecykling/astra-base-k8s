...
| `configure-bgp.py` | Workstation; `NAME --node-ip IP [--node-ip IP ...]` or `NAME --discover`; optional `--dns-forwarding` | Validates the profile, optionally reads controllers from a checked kubeconfig, and prints filtered EdgeOS commands; writes no router/cluster settings and never commits them. See [BGP setup](bgp.md#generate-the-router-setup) |
